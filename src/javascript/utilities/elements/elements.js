import { ElementsCollection } from "./elements-collection.js";
...
class DomElement{
    ...
    getPrevSiblings(){
        const collection = this.element.parentElement.querySelectorAll(':scope > *');
        const i = Array.from(collection).indexOf(this.element);
        const previousSiblings = Array.from(collection).slice(0, i);
        return previousSiblings;
    }

    getIndex(){
        return getIndexOf({of: this.element, inside:this.element.parentElement});
    }
    ...
    getChildsCount(){
        return Array.from(this.element.querySelectorAll(':scope > *')).length;
    }
